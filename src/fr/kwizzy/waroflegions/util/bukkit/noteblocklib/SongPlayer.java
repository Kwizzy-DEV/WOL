...
public abstract class SongPlayer {
    ...
    public void addPlayer(Player p) {
        synchronized (this) {
            if (!playerList.contains(p.getName())) {
                ...
                ArrayList<SongPlayer> songs = SongUtils.playingSongs
                        .get(p.getName());
                if (songs == null) {
                    songs = new ArrayList<SongPlayer>();
                }
                songs.add(this);
                SongUtils.playingSongs.put(p.getName(), songs);
            }
        }
    }
    ...
    public void removePlayer(Player p) {
        synchronized (this) {
            ...
            if (SongUtils.playingSongs.get(p.getName()) == null) {
                return;
            }
            ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(
                    SongUtils.playingSongs.get(p.getName()));
            songs.remove(this);
            SongUtils.playingSongs.put(p.getName(), songs);
